...
class Router {
    ...
    public static function dispatch():void {
        ...
        $action = preg_replace("/\/api_proyecto\/public\//",'',$_SERVER['REQUEST_URI']);//desde el public
       ...
       // $p= preg_match('/[0-9]+$/', $action, $match);  // ESTA ERA LA QUE HABIA
        ...
        if(!empty($match)){
            ...
            if ($param != "login" && $param != "register" && $param != "crear" && $param != "listar" && $param != "modificar" && $param != "cerrar" && $param != "confirmarcuenta") {
